...
// Ikoner (tilpas paths hvis nødvendigt)
...
export default function Library({
  controller,            // <<< brug den fælles controller
  initialQuery = "",
  initialArtist = "",
  focusTrackId = null
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return (controller.tracks || []).filter((t) => {
      if (selectedGenres.size > 0) {
        const gs = Array.isArray(t.genre) ? t.genre : String(t.genre || "").split(",").map(x => x.trim());
  ...
  const isCurrent = (track) => {
    const idx = controller.tracks.indexOf(track);
    return controller.index === idx;
  };
              ...
              return (
                <tr key={t.id || i} ref={(el) => rowRefs.current.set(t.id || i, el)} className={playing ? "isPlaying" : ""}>
                  ...
                  <td className="cellInfo">
                    <button className="infoBtn" onClick={() => setDetailsOf({ track: t })} aria-label="Mere info" title="Mere info">
        ...
        <div className="modalBackdrop" onClick={() => setDetailsOf(null)}>
          <div className="modalCard" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modalGrid">
              <img className="modalCover" src={detailsOf.track.cover?.src} alt="" />
              <div className="modalMeta">
                ...
                <div className="modalList">
                  ...
                  <div><span>Scale:</span> {detailsOf.track.scale || "—"}</div>
                </div>
              </div>
            </div>
          </div>
